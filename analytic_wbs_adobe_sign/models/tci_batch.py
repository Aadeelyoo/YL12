...
class TciBatch(models.Model):
    ...
    def test_pdf_generation(self):
        pdf_report = self.env.ref('analytic_wbs_batch.tci_batch_report').sudo().with_context(adobe_sign=True, display="none").render_qweb_pdf([self.id])[0]
        import time
        import os
    ...
    def get_merged_pdf_for_approval(self):
        ...
        decoded_data.append(pdf_report)
        atts = self.approval_report_ids.mapped('datas')
        atts.insert(0, base64.b64encode(pdf_report or b''))
        merged_pdf = pike_pdf_merge.process_from_stack(atts)
        return base64.b64encode(merged_pdf)
    ...
    def write(self, vals):
            ...
            if 'mail_approval_state' in vals:
                if vals.get('mail_approval_state') == 'review':
                    if self.approval_document_ids and self.mail_user_ids:
                        ...
                        pdf_name = _('{} - Batch-Report - {}.pdf').format(self.external_ref or 'Temporary_File', self.name)
                        ...
                        self.env['adobe.agreement'].start_adobe_document_sign_process(
                            ...
                            destination_field="batch_report_document_temp"
                        )
        ...
        return super(TciBatch, self).write(vals)
    ...
    def create(self, vals):
        ...
        return super(TciBatch, self).create(vals)
    ...
    def approve_related_document(self):
        self.ensure_one()
        self.action_batch_approve()

    def reject_related_document(self, feedback=False):
        self.ensure_one()
        self.action_batch_reject(feedback)