...
class account_analytic_wbs_budget_line(models.Model):
    _name = 'account.analytic_wbs.budget_line'
    ...
    total_amount = fields.Monetary('Total Amount', compute='get_total_amount', store=True)
    contingency = fields.Monetary('Contingency', default=0.0)
    ...
    account_id = fields.Many2one(related="account_project_id.account_id", store=True)
    line_transaction_id = fields.Many2one('account.analytic_wbs.budget_transaction', 'Budget Transaction', required=True, ondelete='cascade')
    bdgt_transaction_state = fields.Selection(related="line_transaction_id.state", string="Transaction State", readonly=True, store=True)
    bdgt_transaction_class = fields.Selection(related="line_transaction_id.transaction_class", string="Transaction Class", readonly=True, store=True)
    ...
    partner_id = fields.Many2one(related="po_id.partner_id", string='Partner', readonly=True, store=True)

    task_id = fields.Many2one('project.task', string="Task", readonly=False, copy=False)
    ...
    tag_ids = fields.Many2many('account.analytic_wbs.tag', 'account_analytic_wbs_budget_line_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True)
    ...
    # Todo: Important - Add link to task and finalyze rep_uif function below to capture budget per task
    @api.depends('po_id', 'employee_id', 'task_id', 'task_id.name', 'po_id.internal_ref', 'account_project_id', 'name')
    def get_repuid(self):
        for rec in self:
            if rec.name and rec.account_project_id:
                wbs = str(rec.account_project_id.name) or False
                if rec.po_id:
                ...
                vals = {
                    "rep_uid": repuid,
                    "rep_uid_type": rep_uid_type,
                    "rep_name": rep_name
                }
                rec.update(vals)

    @api.depends('amount', 'contingency')
    def get_total_amount(self):
        for rec in self:
            rec.total_amount = rec.amount + rec.contingency
    ...
    def unlink(self):
        ...
        for order in self:
            if not user.has_group('analytic_wbs.group_tci_superuser'):
                if order.line_transaction_id.state != 'draft' and order.line_transaction_id.state != 'pending':
                    raise osv.except_osv(('Error'), ('Only "Draft" and "Pending" budget transactions can be modified '))

        return super(account_analytic_wbs_budget_line, self).unlink()


class account_analytic_wbs_budget_transaction(models.Model):
    _name = 'account.analytic_wbs.budget_transaction'
    ...
    def _default_name(self):
        seq_id = self.env['ir.sequence'].get('budget.transaction.number')
        return self.env.context.get('name', seq_id)

    name = fields.Char('Budget Transaction No', required=True, default=_default_name, track_visibility='onchange')
    ...
    transaction_type_id = fields.Many2one('account.analytic_wbs.budget_transaction_type', 'Transaction Type', required=False, ondelete='restrict')
    transaction_detail_ids = fields.One2many('account.analytic_wbs.budget_line', 'line_transaction_id', string='Transaction Detail')
    transaction_class = fields.Selection([
        ('baseline', 'Baseline Budget'),
        ('transfer', 'Budget Transfer'),
        ('trend', 'Contingency Trend Changes'),
    ...
    @api.multi
    def budget_transaction_draft(self):
        self.write({ 'state' : 'draft' })
        return True

    @api.multi
    def budget_transaction_submitted(self):
        self.write({ 'state' : 'submitted' })
        return True

    @api.multi
    def budget_transaction_pending(self):
        self.write({'state': 'pending'})
        return True

    @api.multi
    def budget_transaction_approved(self):
        self.write({'state': 'approved'})
        return True

    @api.multi
    def budget_transaction_posted(self):
        self.write({'state': 'posted'})
        return True

    @api.multi
    def budget_transaction_rejected(self):
        ...
        for order in self:
            if not user.has_group('analytic_wbs.group_tci_superuser'):
                if order.state != 'draft' and order.state != 'pending':
                    raise osv.except_osv(('Error'), ('Only "Draft" and "Pending" budget transactions can be deleted '))
        return super(account_analytic_wbs_budget_transaction, self).unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        return super(account_analytic_wbs_budget_transaction, self)._track_subtype(init_values)

    # Calculate Budget Transaction Man Hours
    @api.multi
    ...
    # Calculate Budget Transaction Amount
    @api.multi
    ...
    # Calculate Budget Transaction Contingency
    @api.multi
    def _compute_transaction_contingency(self):
        analytic_wbs_line_obj = self.env['account.analytic_wbs.budget_line']
        domain = [('line_transaction_id', 'in', self.mapped('id'))]
        if self._context.get('from_date', False):
            domain.append(('date', '>=', self._context['from_date']))
        ...
        transaction_contingencys = analytic_wbs_line_obj.search_read(domain, ['line_transaction_id', 'contingency'])
        line_transaction_ids = set([line['line_transaction_id'][0] for line in transaction_contingencys])
        data_debit_contingency = {line_transaction_id: 0.0 for line_transaction_id in line_transaction_ids}
        data_credit_contingency = {line_transaction_id: 0.0 for line_transaction_id in line_transaction_ids}

        for transaction_contingency in transaction_contingencys:
            if transaction_contingency['contingency'] < 0.0:
                data_debit_contingency[transaction_contingency['line_transaction_id'][0]] += transaction_contingency[
                    'contingency']
            else:
                data_credit_contingency[transaction_contingency['line_transaction_id'][0]] += transaction_contingency[
                    'contingency']

        for account in self:
            account.transaction_contingency_debit = abs(data_debit_contingency.get(account.id, 0.0))
            account.transaction_contingency_credit = data_credit_contingency.get(account.id, 0.0)
            account.transaction_contingency_balance = account.transaction_contingency_credit - account.transaction_contingency_debit

    transaction_contingency_balance = fields.Monetary(compute='_compute_transaction_contingency', string='contingency')
    transaction_contingency_debit = fields.Monetary(compute='_compute_transaction_contingency', string='contingency Debit')
    transaction_contingency_credit = fields.Monetary(compute='_compute_transaction_contingency', string='contingency Credit')


class account_analytic_wbs_budget_transaction_type(models.Model):
    _name = 'account.analytic_wbs.budget_transaction_type'
    _description = 'Budget Transaction Status'
    _order = 'date desc, id desc'
