...
class account_analytic_wbs_progress_line(models.Model):
    _name = 'account.analytic_wbs.progress_line'
    _description = 'wbs Progress Line'
    ...
    line_transaction_id = fields.Many2one('account.analytic_wbs.progress_transaction', 'Progress Transaction', required=True, ondelete='cascade')
    progress_transaction_state = fields.Selection(related="line_transaction_id.state", string="Transaction State", readonly=True, store=True)
    ...
    tag_ids = fields.Many2many('account.analytic_wbs.tag', 'account_analytic_wbs_progress_line_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True)
    ...
    def unlink(self):
        for order in self:
            if order.line_transaction_id.state != 'draft' and order.line_transaction_id.state != 'pending':
                raise osv.except_osv(('Error'), ('Only "Draft" and "Pending" progress transactions can be modified '))
        return super(account_analytic_wbs_progress_line, self).unlink()


class account_analytic_wbs_progress_transaction(models.Model):
    _name = 'account.analytic_wbs.progress_transaction'
    _description = 'Progress Transaction'
    ...
    def _default_name(self):
        seq_id = self.env['ir.sequence'].get('progress.transaction.number')
        return self.env.context.get('name', seq_id)

    name = fields.Char('Progress Transaction No', required=True, default=_default_name, track_visibility='onchange')
    ...
    transaction_type_id = fields.Many2one('account.analytic_wbs.progress_transaction_type', 'Transaction Type', required=False, ondelete='restrict')
    transaction_detail_ids = fields.One2many('account.analytic_wbs.progress_line', 'line_transaction_id', string='Transaction Detail')
    ...
    @api.multi
    def _progress_transaction_draft(self):
        self.write({ 'state' : 'draft' })
        return True

    @api.multi
    def _progress_transaction_submitted(self):
        self.write({ 'state' : 'submitted' })
        return True

    @api.multi
    def _progress_transaction_pending(self):
        self.write({'state': 'pending'})
        return True

    @api.multi
    def _progress_transaction_approved(self):
        self.write({'state': 'approved'})
        return True

    @api.multi
    def _progress_transaction_posted(self):
        self.write({'state': 'posted'})
        return True

    @api.multi
    def _progress_transaction_rejected(self):
    ...
    def unlink(self):
        for order in self:
            if order.state != 'draft' and order.state != 'pending':
                raise osv.except_osv(('Error'), ('Only "Draft" and "Pending" progress transactions can be deleted '))
        return super(account_analytic_wbs_progress_transaction, self).unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        return super(account_analytic_wbs_progress_transaction, self)._track_subtype(init_values)

    # Calculate Progress Transaction Man Hours
    @api.multi
    ...
    # Calculate Progress Transaction Amount
    @api.multi
...
class account_analytic_wbs_progress_transaction_type(models.Model):
    _name = 'account.analytic_wbs.progress_transaction_type'
    _description = 'Progress Transaction Status'
    _order = 'date desc, id desc'
